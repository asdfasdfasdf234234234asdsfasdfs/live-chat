  ...
  return (
    <div className="flex flex-col items-start w-full">
      <button
        ...
        className="w-full flex justify-center items-center h-[52px] bg-usb-red text-white font-medium text-[16px] tracking-wide rounded-lg hover:bg-usb-red-hover transition-colors cursor-pointer border-none"
      >
      ...
      <div className="mt-8 w-full p-5 bg-usb-gray-bg rounded-lg border border-usb-gray-border">
        <h3 className="text-[16px] text-usb-navy mb-3 font-medium">Other ways to contact {loading ? "us" : regionData.brandName}</h3>
        
        <div className="flex flex-col gap-2 mt-4">
          <p className="text-[14px] text-usb-text-secondary mb-1">Telephone Banking Support:</p>
          <p className="font-medium text-[18px] text-usb-navy">
             {loading ? "Loading..." : regionData.supportPhone}
          </p>
          <p className="text-[12px] text-usb-input-border mt-1">Lines are open 24/7. Standard rates apply.</p>
        </div>
      </div>
    </div>
  );
